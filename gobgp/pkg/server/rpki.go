...
var rtr_time int64
var time_transport, start_transport int64
...
func (m *roaManager) handleRTRMsg(client *roaClient, state *config.RpkiServerState, buf []byte) {
	...
	rtr_start_time := time.Now().UnixNano()
	m1, err := rtr.ParseRTR(buf)
	rtr_end_time := time.Now().UnixNano()
	...
	if err == nil {
		...
		case *rtr.RTRCacheResponse:
			...
			client.endOfData = false
		case *rtr.MzrRTRIPPrefix:
			rtr_time += (rtr_end_time - rtr_start_time)

			for _, p := range msg.RTRIPPrefixList {
				family := bgp.AFI_IP
				if p.Type == rtr.RTR_MZR_IPV4_PREFIX {
					received.Ipv4Prefix++
				} else {
					family = bgp.AFI_IP6
					received.Ipv6Prefix++
				}
				roa := table.NewROA(family, p.Prefix, p.PrefixLen, p.MaxLen, p.AS, client.host)
				if (p.Flags & 1) == 1 {
					if client.endOfData {
						m.table.Add(roa)
					} else {
			...
			//log.Debugf("extract ipv4 prefix num: %d, ipv6 prefix num: %d", received.Ipv4Prefix, received.Ipv6Prefix)
		case *rtr.RTRIPPrefix:
			rtr_time += (rtr_end_time - rtr_start_time)
			//fmt.Printf("rtr time : %v \n", rtr_time)
		...
		case *rtr.RTREndOfData:
			end_transport := time.Now().UnixNano()
			time_transport = end_transport - start_transport
			//fmt.Printf("rtr time : %v \n", rtr_time)
			fmt.Printf("transport time : %v\n", time_transport)

			fmt.Printf("ipv4 decode time : %v\nipv6 decode time : %v\n", rtr.V4_decode_time, rtr.V6_decode_time)

			/*f1, err := os.OpenFile("/home/cnic/rpki/111test/compress/current/ipv4_decode_time.txt", os.O_CREATE|os.O_RDWR|os.O_APPEND, 0755)
			defer f1.Close()
			if err != nil {
			...
			f, err := os.OpenFile("/home/cnic/rpki/111test/compress/current/ipv6_decode_time.txt", os.O_CREATE|os.O_RDWR|os.O_APPEND, 0755)
			defer f.Close()
			if err != nil {
				fmt.Printf("openfile wrong !!!!\n")
			}
			...
			//f3, err := os.OpenFile("/home/cnic/rpki/111test/unlimit_transport/hROA_current_time.txt", os.O_CREATE|os.O_RDWR|os.O_APPEND, 0755)
			f3, err := os.OpenFile("/home/cnic/rpki/111test/tROA/current/transport_time.txt", os.O_CREATE|os.O_RDWR|os.O_APPEND, 0755)
			defer f3.Close()
			if err != nil {
				fmt.Printf("openfile wrong !!!!\n")
			}
...
func (c *roaClient) softReset() error {
	if c.conn != nil {
		/*fmt.Printf("NewRTRResetQuery !!!!! \n")
		start_time := time.Now().UnixNano()
		fmt.Printf("NewRTRResetQuery time is %v \n", start_time)*/
...
func (c *roaClient) established() (err error) {
	...
	start_transport = time.Now().UnixNano()
	fmt.Printf("establish time is : %v\n", start_transport)
