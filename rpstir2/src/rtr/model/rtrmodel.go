...
const (
	...
	PDU_TYPE_MZR_IPV4_PREFIX = 11
	PDU_TYPE_MZR_IPV6_PREFIX = 12
...
type MzrRtrIpv6PrefixModel struct {
	ProtocolVersion uint8   `json:"protocolVersion"`
	PduType         uint8   `json:"pduType"`
	Zero0           uint16  `json:"zero0"`
	Length          uint32  `json:"length"`
	EncodedAddress	uint128.Uint128	`json:"encodedAddress"`
	Bitmap			uint32	`json:"bitmap"`
	Asn             uint32  `json:"asn"`
}

func (m *MzrRtrIpv6PrefixModel) Bytes() []byte {
	wr := bytes.NewBuffer([]byte{})
	binary.Write(wr, binary.BigEndian, m.ProtocolVersion)
	binary.Write(wr, binary.BigEndian, m.PduType)
	binary.Write(wr, binary.BigEndian, m.Zero0)
	binary.Write(wr, binary.BigEndian, m.Length)
	encodedAddress := m.EncodedAddress.Bytes()
	wr.Write(encodedAddress)
	binary.Write(wr, binary.BigEndian, m.Bitmap)
	binary.Write(wr, binary.BigEndian, m.Asn)
	return wr.Bytes()
}

func (m *MzrRtrIpv6PrefixModel) PrintBytes() string {
	return convert.PrintBytes(m.Bytes(), 8)
}

func (m *MzrRtrIpv6PrefixModel) GetProtocolVersion() uint8 {
	return m.ProtocolVersion
}

func (m *MzrRtrIpv6PrefixModel) GetPduType() uint8 {
	return m.PduType
}

func NewMzrRtrIpv6PrefixModel(protocolVersion uint8, encodedAddress uint128.Uint128, bitmap, asn uint32) *MzrRtrIpv6PrefixModel {
	return &MzrRtrIpv6PrefixModel{
		ProtocolVersion: protocolVersion,
		PduType:         PDU_TYPE_MZR_IPV6_PREFIX,
		Zero0:           0,
		Length:          32,
...
type MzrRtrIpv4PrefixModel struct {
	ProtocolVersion uint8   `json:"protocolVersion"`
	PduType         uint8   `json:"pduType"`
	Zero0           uint16  `json:"zero0"`
	Length          uint32  `json:"length"`
	EncodedAddress	uint32	`json:"encodedAddress"`
	Bitmap			uint32	`json:"bitmap"`
	Asn             uint32  `json:"asn"`
}


func NewMzrRtrIpv4PrefixModel(protocolVersion uint8, encodedAddress, bitmap, asn uint32) *MzrRtrIpv4PrefixModel {
	return &MzrRtrIpv4PrefixModel{
		ProtocolVersion: protocolVersion,
		PduType:         PDU_TYPE_MZR_IPV4_PREFIX,
		Zero0:           0,
		Length:          20,
		EncodedAddress:  encodedAddress,
		Bitmap:          bitmap,
...
func (m *MzrRtrIpv4PrefixModel) Bytes() []byte {
	wr := bytes.NewBuffer([]byte{})
	binary.Write(wr, binary.BigEndian, m.ProtocolVersion)
	binary.Write(wr, binary.BigEndian, m.PduType)
	binary.Write(wr, binary.BigEndian, m.Zero0)
	binary.Write(wr, binary.BigEndian, m.Length)
	binary.Write(wr, binary.BigEndian, m.EncodedAddress)
	binary.Write(wr, binary.BigEndian, m.Bitmap)
	binary.Write(wr, binary.BigEndian, m.Asn)
	return wr.Bytes()
}

func (m *MzrRtrIpv4PrefixModel) PrintBytes() string {
	return convert.PrintBytes(m.Bytes(), 8)
}

func (m *MzrRtrIpv4PrefixModel) GetProtocolVersion() uint8 {
	return m.ProtocolVersion
}

func (m *MzrRtrIpv4PrefixModel) GetPduType() uint8 {
	return m.PduType
}
