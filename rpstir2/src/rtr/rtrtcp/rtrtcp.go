...
func ProcessRtrPduModel(buf *bytes.Reader, rtrPduModel rtrmodel.RtrPduModel) (rtrResponse []rtrmodel.RtrPduModel, err error) {
	...
	case rtrmodel.PDU_TYPE_RESET_QUERY:
		var resetResponse []rtrmodel.RtrPduModel
		switch conf.String("mzr::use") {
		case "chenbo":
			resetResponse, err = MzrProcessResetQuery(rtrPduModel)
		case "compress":
			resetResponse, err = CompressProcessResetQuery(rtrPduModel)
		case "flex":
		case "zipped_fixed":
		case "zipped_flex":
		default:
