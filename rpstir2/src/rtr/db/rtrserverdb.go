...
func MzrGetIpv4RtrFullRawAndSessionIdAndSerialNumber() (rtrFulls []model.MzrLabRpkiRtrRawData, sessionId uint16, serialNumber uint32, err error) {
	/*
		sql := `select id, serialNumber, asn,address, prefixLength,maxLength
		from lab_rpki_rtr_full order by id`
	*/
	err = xormdb.XormEngine.Table("mzr_rpki_mroa_ipv4_cur_rtr_full").Cols("id, asn, address, prefixLength, maxLength").
		OrderBy("id").Find(&rtrFulls)

	if err != nil {
		belogs.Error("MzrGetRtrFullAndSerialNumAndSessionId():select  mzr_rpki_mroa_ipv4_cur_rtr_full fail:", err)
		return rtrFulls, sessionId, serialNumber, err
	}
	belogs.Debug("MzrGetRtrFullAndSerialNumAndSessionId():select mzr_rpki_mroa_ipv4_cur_rtr_full, len :", len(rtrFulls))

	return rtrFulls, 0, 0, nil
}

func MzrGetIpv6RtrFullRawAndSessionIdAndSerialNumber() (rtrFulls []model.MzrLabRpkiRtrRawData, sessionId uint16, serialNumber uint32, err error) {
	/*
		sql := `select id, serialNumber, asn,address, prefixLength,maxLength
		from lab_rpki_rtr_full order by id`
	*/
	err = xormdb.XormEngine.Table("mzr_rpki_mroa_ipv6_cur_rtr_full").Cols("id, asn, address, prefixLength, maxLength").
		OrderBy("id").Find(&rtrFulls)

	if err != nil {
		belogs.Error("MzrGetRtrFullAndSerialNumAndSessionId():select  mzr_rpki_mroa_ipv6_cur_rtr_full fail:", err)
		return rtrFulls, sessionId, serialNumber, err
	}
	belogs.Debug("MzrGetRtrFullAndSerialNumAndSessionId():select mzr_rpki_mroa_ipv6_cur_rtr_full, len :", len(rtrFulls))
